  ...
  return `
    <div class="explorer">
      ...
      <section class="explorer-content" aria-labelledby="explorer-section-title">
        ...
        ${renderBlocks(section, collectedKeys)}
      </section>
    </div>
  `;
}

function renderBlocks(section, collectedKeys) {
    ...
    .map((block) => {
            ...
            return `
              <article class="explorer-record">
                ...
                <div class="data-tag-grid">
                  ${renderFields(record.fields ?? [], section, collectedKeys)}
                </div>
              </article>
            `;
          })
      ...
      return `
        <div class="explorer-block">
          ${block.title ? `<h3>${escapeHtml(block.title)}</h3>` : ''}
          <div class="data-tag-grid">
            ${renderFields(block.fields ?? [], section, collectedKeys)}
          </div>
        </div>
      `;
    })
...
function renderFields(fields, section, collectedKeys) {
  return fields
    .map((field) => {
      if (field.usable) {
        return DataTag({
